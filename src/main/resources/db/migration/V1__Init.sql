...
CREATE TABLE IF NOT EXISTS auto_translation_previews (
                                                         auto_translation_preview_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                                         token VARCHAR(64) NOT NULL,
    admin_user_id INTEGER NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    comic_id INTEGER NOT NULL REFERENCES comics(comic_id) ON DELETE CASCADE,
    source_language_id INTEGER NOT NULL REFERENCES languages(language_id),
    target_language_id INTEGER NOT NULL REFERENCES languages(language_id),
    is_consumed BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    expires_at TIMESTAMP,
    CONSTRAINT uq_auto_translation_previews_token UNIQUE (token)
    );

CREATE TABLE IF NOT EXISTS auto_translation_preview_pages (
                                                              auto_translation_preview_page_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                                              preview_id INTEGER NOT NULL REFERENCES auto_translation_previews(auto_translation_preview_id) ON DELETE CASCADE,
    ...
    CONSTRAINT uq_auto_translation_preview_pages_preview_page UNIQUE (preview_id, page_number)
    );
...
CREATE INDEX IF NOT EXISTS idx_auto_translation_previews_admin
    ON auto_translation_previews(admin_user_id);

CREATE INDEX IF NOT EXISTS idx_auto_translation_preview_pages_preview
    ON auto_translation_preview_pages(preview_id);
