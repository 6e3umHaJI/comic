...
<body data-authenticated="<c:out value='${pageContext.request.userPrincipal != null}'/>">
<div class="wrapper">
            ...
            <div class="card-scroller" tabindex="0">
                <c:forEach var="comic" items="${popularComics}">
                    <a class="card-tile" href="<c:url value='/comics/${comic.id}'/>">
                        ...
                        <div class="tile-title" title="<c:out value='${comic.title}'/>">
            ...
            <div class="updates-list">
                <c:forEach var="item" items="${recentUpdates}">
                    <a class="update-row" href="<c:url value='/read/${item.translationId}'/>">
                        <div class="u-cover">
                            <img src="<c:url value='/assets/covers/${item.comicCover}'/>"
                                 alt="<c:out value='${item.comicTitle}'/>">
                        </div>

                        <div class="u-main">
                            <div class="u-title">
                                <c:out value="${item.comicTitle}"/>
                            </div>

                            <div class="u-meta">
                                <b>Глава: </b><c:out value="${item.chapterNumber}"/>
                                <b>Язык: </b><c:out value="${item.languageName}"/>
                                <b>Добавлено: </b>
                                <time class="u-date" datetime="<c:out value='${item.createdAtIso}'/>">
            ...
            <div class="grid-tiles">
                <c:forEach var="comic" items="${newComics}">
                    <a class="grid-card" href="<c:url value='/comics/${comic.id}'/>">
                        ...
                        <div class="g-sub">
                            Добавлен:
                            <time class="u-date" datetime="<c:out value='${comic.createdAtIso}'/>">
                                <c:out value="${comic.createdAtFormatted}"/>
                            </time>
                        </div>
                    </a>
                </c:forEach>
